...
|**Request Body**| {<br>&emsp;&emsp;"executionId": "<test\_plan\_id>",<br>&emsp;&emsp;"executionConfig" : {<br>&emsp;&emsp;"name": "config 2"<br>&emsp;&emsp;OR<br>&emsp;&emsp;"id": "2"<br>&emsp;&emsp;}<br>&emsp;&emsp;"uploadVersions": {<br>&emsp;&emsp;&emsp;&emsp;"<Device\_ID>": "<UPLOAD\_VERSION\_ID> , <Device\_ID>": "<UPLOAD\_VERSION\_ID>"<br>&emsp;&emsp;}<br>}<br>Get Device ID from the Test Development -> Test Plan Details page ->Test Devices<br><br/>Get the <ID> from the  GET Upload Version API, this is the version of upload you want to use on the test plan  <br> ![Device ID from Test Plan Details Page](https://docs.testsigma.com/images/api/test-plans/test-plan-device-id.png) <br>Execution Config name and ID are the saved configurations for [Partial Test Plan Runs](https://testsigma.com/docs/runs/test-plan-executions/#partial-test-runs). In the POST request, you can either use the name <br>or the ID for the Execution Config.<br><br>Get the Execution Config ID and name via this GET Request: https://testsigma.com/docs/api/test-plans/#get-test-plan-favouritesconfig<br>|
