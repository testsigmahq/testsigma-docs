...
4. In the **Copilot** overlay, select the **Test Lab** to run the session on: **Testsigma Cloud Lab** or **Local Devices**.
   ![Copilot](https://s3.amazonaws.com/static-docs.testsigma.com/new/projects/applications/Copilot_Cloud.png)

   - **Testsigma Cloud Lab**: Select a **Browser** from the dropdown. The browser list is populated from the cloud lab. The Testsigma Terminal is not required, and OS and resolution are not configurable for this lab.

   - **Local Devices**: Make sure the **Testsigma Terminal** is running, then select a **Connected Machine** and a **Browser**. The **Copilot Status** indicator shows the readiness of the local agent connection and appears only for Local Devices.

[[info | **NOTE**:]]
| - A session on the **Testsigma Cloud Lab** occupies one cloud parallel for its duration, and the parallel is released only when the session ends. If no cloud parallel is available, you cannot launch the session.
...
| - Runs performed through a cloud session are recorded as **Dry Run** on the parallel usage, same as other dry runs.

5. In the **Copilot** overlay, click the dropdown in the **Set Initial Debug Point** field and select the step at which you want execution to pause automatically.
...
6. If needed, click the dropdown in the **Execute from Step** field and select the step from which you want execution to begin, skipping all prior steps.
