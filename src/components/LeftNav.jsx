...
class ListItem extends React.Component {
  ...
  setActive = (name) => {
    ...
    if (active.indexOf(name) === -1) {
      ...
      if (!expandedPanels.includes(name)) {
        // Check if name is already in expandedPanels
        this.setState((prev) => ({
          expandedPanels: [...prev.expandedPanels, name],
        }));
      }
    }
  };
    ...
    return (
      url.indexOf('/overview/') === -1 && (
        ...
        >
          ...
          <img src='https://s3.amazonaws.com/website-static-docs.testsigma.com/new_images/projects/Updated_Doc_Images/bookimg.png' alt='book' loading='eager' />
  ...
  parent = (data, name) => {
    ...
    return (
      <ul
        key={`${name}-${uuidv4()}`}
        className={` ${
          this.state.expandedPanels?.indexOf(name) !== -1
            ? 'active'
            : 'inactive'
        } ${isRoot ? ' root' : ''} `}
      >
        <li
          data-parent={this.state.identifier}
          className={`parent${
            this.inUrl(`/${this.state.identifier || 'docs'}/${name}/`)
        ...
        >
          ...
          <svg
            ...
            identifier={name}
            className={`-mr-3 inline caret${
              this.isActive(name) ? ' active-caret' : ''
            }`}
            xmlns='http://www.w3.org/2000/svg'
            fill='none'
            height='24'
